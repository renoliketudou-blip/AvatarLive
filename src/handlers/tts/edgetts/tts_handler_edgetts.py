        ...
        if not text_end:
            ...
            if len(sentences) > 1:  # 至少有一个完整句子
                ...
                for sentence in complete_sentences:
                    ...
                    t1 = time.monotonic()
                    logger.info(
                        f"TTS TIMING: sentence synth {(t1-t0)*1000:.0f}ms '{sentence[:20]}' "
                        f"({len(data)} bytes)"
                    )
            ...
            if context.input_text is not None and len(context.input_text.strip()) > 0:
                    t0 = time.monotonic()
                    logger.info(f"TTS TIMING: start final synth '{context.input_text[:30]}'")
                    ...
                    t1 = time.monotonic()
                    logger.info(
                        f"TTS TIMING: final synth done {(t1-t0)*1000:.0f}ms "
                        f"'{context.input_text[:20]}' ({len(data)} bytes) -> FlashHead"
                    )
                    output_audio = librosa.load(io.BytesIO(data), sr=None)[0]
                    output_audio = output_audio[np.newaxis, ...]
                    output = DataBundle(output_definition)
