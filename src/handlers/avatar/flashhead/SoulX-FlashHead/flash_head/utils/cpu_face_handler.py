...
class CPUFaceHandler:
    ...
    def __init__(self, model_selection: int = 1, min_detection_confidence: float = 0.0):
        ...
        # mediapipe is optional: only needed when face cropping (use_face_crop=true)
        # is enabled. Import lazily so the streaming pipeline (use_face_crop=false)
        # works without mediapipe installed.
        try:
            import mediapipe as mp
        except ImportError:
            raise ImportError(
                "mediapipe is required only when use_face_crop=true. "
                "Install it with `pip install mediapipe`, or disable face cropping."
            )
