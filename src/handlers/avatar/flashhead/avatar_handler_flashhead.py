...
class HandlerAvatarFlashHead(HandlerBase):
    ...
    def __init__(self) -> None:
        ...
        # Paste-back state (v3-style full-frame composition)
    ...
    def load(self, engine_config: ChatEngineConfigModel,
        ...
        # paste_back implies face cropping + pasting the generated face back
        # onto the full-frame condition image (v3-style composition).
        self._orig_cond_image_path = cond_image_path
        if handler_config.use_face_crop or handler_config.paste_back:
            from handlers.avatar.flashhead.flashhead_face_crop import crop_face_with_box
            crop_box = None
            try:
                cropped, crop_box, _bbox = crop_face_with_box(cond_image_path)
                cropped_path = cond_image_path + ".cropped.png"
                cropped.save(cropped_path)
                logger.info(
                    f"FlashHead: face-cropped image saved to {cropped_path} "
                    f"(crop_box={crop_box})"
                )
            ...
            if handler_config.paste_back:
                from PIL import Image as _PILImage
                if crop_box is not None:
                    try:
                        bg_pil = _PILImage.open(self._orig_cond_image_path).convert("RGB")
                        bg_bgr = np.array(bg_pil)[:, :, ::-1].copy()  # RGB -> BGR
                        self._paste_back_bg = bg_bgr
                        self._paste_back_box = crop_box
                        logger.info(
                            f"FlashHead: paste-back enabled bg={bg_bgr.shape} box={crop_box}"
                        )
                    except Exception as e:
                        logger.warning(
                            f"FlashHead: paste-back bg load failed ({e}), disabled"
                        )
                        self._paste_back_bg = None
                        self._paste_back_box = None
                else:
                    self._paste_back_bg = None
                    self._paste_back_box = None
    ...
    def create_context(self, session_context: SessionContext,
        ...
        processor = FlashHeadProcessor(
            ...
            paste_back_bg=self._paste_back_bg,
            paste_back_box=self._paste_back_box,
            paste_back_feather=handler_config.paste_back_feather,
        )
