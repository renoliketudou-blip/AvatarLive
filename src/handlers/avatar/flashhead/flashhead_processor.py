...
class FrameQueueItem:
    ...
    end_of_speech: bool
    created_at: float = field(default_factory=time.monotonic)  # enqueue time (timing log)
...
class FlashHeadProcessor:
    ...
    def __init__(
        ...
        paste_back_bg: Optional[np.ndarray] = None,
        paste_back_box: Optional[tuple] = None,
        paste_back_feather: int = 30,
    ):
        ...
        # --- Paste-back (v3-style full-frame composition) ---
        # paste_back_bg: BGR uint8 full-frame background image.
        # paste_back_box: (x1, y1, x2, y2) region in bg that the generated
        #                 512x512 face maps to (the face crop region).
        self._paste_back_bg = paste_back_bg
        self._paste_back_box = paste_back_box
        self._paste_back_feather = int(paste_back_feather)
        self._paste_back_mask: Optional[np.ndarray] = None
        self._pb_w = 0
        self._pb_h = 0
        if paste_back_bg is not None and paste_back_box is not None:
            self._prepare_paste_back()
        ...
        # --- Timing instrumentation (speech latency) ---
        self._speech_start_mono: Optional[float] = None  # first TTS audio arrival
        self._first_frame_logged = False
        self._last_backlog_log: Optional[float] = None  # throttle queue-wait logs
    ...
    def _make_idle_frame(self, pipeline) -> np.ndarray:
        ...
        bgr = rgb[:, :, ::-1].copy()
        bgr = self._apply_paste_back(bgr)
        logger.info(f"FlashHead: idle frame prepared, shape={bgr.shape}")
        return bgr

    def _prepare_paste_back(self):
        """Precompute the feathered alpha mask over the paste-back region."""
        x1, y1, x2, y2 = self._paste_back_box
        self._pb_w = int(x2 - x1)
        self._pb_h = int(y2 - y1)
        if self._pb_w < 4 or self._pb_h < 4:
            logger.warning("FlashHead: paste-back box too small, disabling paste-back")
            self._paste_back_bg = None
            return
        f = min(self._paste_back_feather, self._pb_w // 3, self._pb_h // 3)
        core = np.zeros((self._pb_h, self._pb_w), dtype=np.uint8)
        core[f:self._pb_h - f, f:self._pb_w - f] = 1
        mask = cv2.GaussianBlur(core.astype(np.float32), (0, 0), sigmaX=max(1.0, f / 2.0))
        self._paste_back_mask = mask.astype(np.float32)
        logger.info(
            f"FlashHead: paste-back prepared bg={self._paste_back_bg.shape} "
            f"box={self._paste_back_box} feather={f}"
        )

    def _apply_paste_back(self, frame_bgr: np.ndarray) -> np.ndarray:
        """Resize the generated face back onto the full-frame background.

        frame_bgr: BGR uint8 (512x512) generated face. Returns a full-frame
        BGR uint8 image (e.g. 720x1280) when paste-back is enabled, otherwise
        the input unchanged.
        """
        bg = self._paste_back_bg
        if bg is None or self._paste_back_mask is None:
            return frame_bgr
        x1, y1, x2, y2 = self._paste_back_box
        face = cv2.resize(frame_bgr, (self._pb_w, self._pb_h),
                          interpolation=cv2.INTER_LINEAR)
        out = bg.copy()
        region = out[y1:y1 + self._pb_h, x1:x1 + self._pb_w].astype(np.float32)
        alpha = self._paste_back_mask[..., None]
        out[y1:y1 + self._pb_h, x1:x1 + self._pb_w] = (
            region * (1.0 - alpha) + face.astype(np.float32) * alpha
        ).astype(np.uint8)
        return out
    ...
    def add_audio(self, audio_data_16k: np.ndarray, original_audio: np.ndarray,
        ...
        if not was_speaking:
            ...
            self._first_frame_logged = False
            logger.info(f"FlashHead TIMING: TTS audio received (speech_id={speech_id}) — waiting for first frame")
        ...
        if end_of_speech:
            ...
            # Speech fully processed: reset audio deque & latent to silence so the
            # idle worker generates a clean closed-mouth idle (no residual speech
            # tail driving a wide-open mouth right after speaking).
            cached_audio_length = self.sample_rate * self.cached_audio_duration
            self._audio_deque = deque([0.0] * cached_audio_length, maxlen=cached_audio_length)
            self._latent_motion_frames = self._initial_latent_1slice.clone()
            with self._lock:
                self._speaking = False
            if self._speech_start_mono is not None:
                total_ms = (time.monotonic() - self._speech_start_mono) * 1000
                logger.info(
                    f"FlashHead TIMING: speech flush done, tts_audio→done {total_ms:.0f}ms "
    ...
    def _process_chunk(self, audio_chunk_16k: np.ndarray, original_audio: np.ndarray,
        ...
        try:
            ...
            elapsed = ((time.monotonic() - self._speech_start_mono) * 1000
                       if self._speech_start_mono is not None else 0.0)
            logger.info(
                f"FlashHead TIMING: chunk inf {video_frames.shape[0]}fr in {dur_ms:.0f}ms "
                f"({video_frames.shape[0] / (dur_ms / 1000):.0f}FPS) "
                f"[tts_audio+{elapsed:.0f}ms] speech_id={speech_id}"
            )
        finally:
        ...
        for i in range(n_frames):
            ...
            frame_bgr = self._apply_paste_back(frame_bgr)
    ...
    def _frame_collector_worker(self):
        ...
        while not self._stop_event.is_set():
            ...
            if item is not None and item.video_frame is not None:
                # Timing log: first speaking frame latency + queue backlog
                if item.speech_id is not None:
                    now = time.monotonic()
                    q_wait = (now - item.created_at) * 1000
                    if self._speech_start_mono is not None and not self._first_frame_logged:
                        self._first_frame_logged = True
                        total = (now - self._speech_start_mono) * 1000
                        logger.info(
                            f"FlashHead TIMING: FIRST speaking frame emitted: "
                            f"tts_audio→frame {total:.0f}ms (queue_wait {q_wait:.0f}ms) "
                            f"speech_id={item.speech_id}"
                        )
                    elif q_wait > 80:
                        # Throttle backlog logging to ~1/sec to avoid log flooding
                        if (self._last_backlog_log is None
                                or (now - self._last_backlog_log) > 1.0):
                            self._last_backlog_log = now
                            logger.info(
                                f"FlashHead TIMING: frame queue_wait {q_wait:.0f}ms (backlog) "
                                f"speech_id={item.speech_id}"
                            )
                # Speaking frame
