...
class FlashHeadConfig(HandlerBaseConfigModel, BaseModel):
    ...
    paste_back: bool = Field(
        default=False,
        description="Paste the generated face back onto the original full-frame condition "
                    "image (v3-style full-body composition, face smaller in frame). "
                    "Implies use_face_crop. Output video size = original image size.",
    )
    paste_back_feather: int = Field(
        default=30,
        description="Feather (soft edge) width in pixels for the paste-back seam.",
    )
