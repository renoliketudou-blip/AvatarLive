...
def crop_face_with_box(
    ...
    """Detect and crop the face region from an image, returning the crop geometry.

    Returns (cropped_Image, crop_box, bbox) where:
      - cropped: PIL Image resized to target_size (the face region to generate).
      - crop_box: (x1, y1, x2, y2) region in the ORIGINAL image that the generated
        face corresponds to — used for paste-back.
      - bbox: (x1, y1, x2, y2) face bounding box in the ORIGINAL image.
...
def crop_face(
    ...
    Backwards-compatible wrapper around :func:`crop_face_with_box`.
    """
    cropped, _crop_box, _bbox = crop_face_with_box(image_path, face_ratio, target_size)
    return cropped
