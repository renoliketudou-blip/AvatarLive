    ...
    def handle(self, context: HandlerContext, inputs: ChatData,
        ...
        if audio is not None:
            ...
            if len(context.output_audios) == 0:
                # First audio chunk of a new speech segment
                context._asr_seg_start = time.monotonic()
        ...
        t1 = time.monotonic()
        seg_dur = (t1 - getattr(context, "_asr_seg_start", t0)) * 1000
        logger.info(
            f"ASR TIMING: recognize {(t1-t0)*1000:.0f}ms for {len(output_audio)/16000:.2f}s "
            f"audio [speech_start→result {seg_dur:.0f}ms]"
        )
        logger.info(res)
