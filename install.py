...
# Skip packages that need nvcc/toolchain to build (flash-attn); we rely on SDPA fallback.
SKIP_PKGS: set = {"flash-attn", "xformers", "mediapipe"}
    ...
    for handler_name, handler_dir in handler_dirs.items():
        ...
        for dep_str in raw_deps:
            ...
            if norm_name in {normalize_pkg_name(s) for s in MUSETALK_SPECIAL_PKGS}:
                continue
            if norm_name in {normalize_pkg_name(s) for s in SKIP_PKGS}:
                continue
...
def main():
    ...
    if not args.configs and not args.install_all:
        ...
        print("Examples:")
        print("  uv run install.py --config config/chat_with_openai_compatible_bailian_cosyvoice_flashhead.yaml")
